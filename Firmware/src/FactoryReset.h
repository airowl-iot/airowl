...
void setResetComplete() {
    ...
    if (httpCode > 0) {
        if (httpCode == HTTP_CODE_OK) {
            Serial.println("[VA-F] Factory reset status updated successfully");
        } else {
            Serial.printf("[VA-F] Factory reset status update failed with code: %d\n", httpCode);
        }
    } else {
...
void factoryResetDevice() {
       Serial.println("[VA-F] Factory reset device");
       
       // Erase the NVS partition
       esp_err_t err = nvs_flash_erase();
       if (err != ESP_OK) {
           Serial.printf(" [VA-F] Error erasing NVS: %d\n", err);
           return;
       }
       ...
       err = nvs_flash_init();
       if (err != ESP_OK) {
           Serial.printf("[VA-F] Error initializing NVS: %d\n", err);
           return;
       }
   }