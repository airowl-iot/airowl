...
bool SensorManager::init(const SensorConfig& pmsConfig, const SensorConfig& ahtConfig, const SensorConfig& ens160Config) {
    ...
    initialized = pmsInitialized || ahtInitialized || ens160Initialized;
    return initialized;
}
...
bool SensorManager::start() {
    ...
    running = true;
    Serial.println("[Sensor Manager] PMS, AHT and ENS160 started");
    return true;
}
...
void SensorManager::updateConfig(const SensorConfig& pmsConfig, const SensorConfig& ahtConfig, const SensorConfig& ens160Config) {
    ...
    if (running) {
        if (pmsConfig.enabled && !pmsWasEnabled) {
            ...
            // HAL::PMS::sleep();
            Serial.println("[Sensor Manager] PMS 1");
        }
    ...
    if (currentTime - lastPMSRead >= pmsConfig.readInterval) {
       ...
       if (HAL::PMS::read(&lastPMSData) == HAL::PMS::Error::NONE) {
            if (HAL::PMS::isDataAvailable()) {
                // Get the data
                // lastPMSData = HAL::PMS::getData();
                Serial.println("[Sensor Manager] PMS data available");
                newPMSData = true;
        ...
        lastAHTRead = currentTime;
         ...
         if (HAL::AHT::read(&lastAHTData) == HAL::AHT::Error::NONE) {
            if (HAL::AHT::isDataAvailable()) {
                // Get the data
                // lastAHTData = HAL::AHT::getData();
                Serial.println("[Sensor Manager] AHT data available");
                newAHTData = true;
