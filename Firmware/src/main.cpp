...
void setup()
{
  ...
  if (ret == ESP_OK) Serial.println("[BOOT] NVS initialized successfully");
  else Serial.printf("[BOOT] ERROR: NVS initialization failed with code: 0x%x\n", ret);
  ...
  if (!APP::SensorManager::init()) Serial.println("[BOOT] SensorManager init failed!");
  else
  {
  ...
  if (!APP::UIController::init()) Serial.println("[BOOT] UIController init failed!");
  else
  {
    Serial.println("[BOOT] UIController initialized successfully");
    if (APP::UIController::startTask()) Serial.println("[BOOT] UIController task started");
    else Serial.println("[BOOT] Failed to start UIController task");
  }

  if (!APP::ServiceManager::getInstance().init()) Serial.println("[BOOT] ServiceManager init failed!");
  else
  {
    if (!APP::ServiceManager::getInstance().start()) Serial.println("[BOOT] Failed to start services!");
  }
